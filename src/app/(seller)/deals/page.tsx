...
export default function SellerDealsPage() {
  ...
  useEffect(() => {
    const fetchDeals = async () => {
      setLoading(true);
      const result = await getActiveDeals("seller");
      if (result.success) {
        setDeals(result.deals || []);
      } else {
        setError(result.error || "Failed to load active sale transaction records.");
      }
      setLoading(false);
    };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 font-sans space-y-8">
      <div className="border-b border-black/[0.06] pb-5 space-y-1">
        <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-brand-green bg-emerald-50 border border-emerald-100 px-2 py-0.5 inline-block">
          SELLER FINANCIAL REGISTRY
        </span>
        <h1 className="text-2xl font-serif italic font-black text-brand-dark">Active Assets Sale Deals</h1>
        <p className="text-xs text-brand-dark/60">
          List of digital businesses and enterprise assets you are selling currently in due diligence, contract signing, or escrow payouts.
        </p>
      </div>
      ...
      {deals.length === 0 ? (
        <div className="border border-black/[0.08] bg-brand-cream/5 p-12 text-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="font-serif italic font-bold text-brand-dark text-base">No Active Sales</h3>
            <p className="text-xs text-brand-dark/50 max-w-sm mx-auto">
              You do not have any active sale transaction agreements. Sales are initialized when you accept a buyer's submitted purchase offer.
            </p>
          </div>
          <button
          ...
          {deals.map((deal) => {
            ...
            return (
              ...
              >
                ...
                <div className="space-y-3.5">
                  ...
                  <div className="space-y-2 text-xs font-mono text-brand-dark/60 border-t border-black/[0.04] pt-3.5">
                    <div className="flex justify-between items-center">
                      <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5 text-brand-dark/40" /> Buyer Name:</span>
                ...
                {/* Action button */}
                <button
                  onClick={() => window.location.hash = `#/seller/deals/${deal.id}`}
