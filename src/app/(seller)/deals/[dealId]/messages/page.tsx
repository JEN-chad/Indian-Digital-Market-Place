...
export function SellerDealMessagesTab({ dealId, deal }: DealSubpageProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Pusher subscribe failed in SellerDealMessagesTab:", e);
    }
        ...
        {messagesList.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-2">
            ...
            <p className="text-[11px] text-brand-dark/50 max-w-xs">
              This channel is encrypted. All conversations are covered by the mutual NDA. Type a message below to coordinate handover.
            </p>
          </div>
        ) : (
                ...
                <div className={`max-w-xs sm:max-w-md p-3 border space-y-1 ${
                  ...
                  <div className="flex items-center gap-1.5 justify-between">
                    <span className={`text-[9px] font-mono font-bold tracking-wider uppercase ${isMine ? "text-brand-green" : "text-brand-dark/50"}`}>
                      {isMine ? "YOU (SELLER)" : msg.senderName || "BUYER"}
                    </span>
